...
class RangeSlider(QtGui.QWidget, Widget):
    _style = None
    _instances = set()
    def __init__(self,label=None, parent=None,image=None):
        super(RangeSlider, self).__init__()

        Widget.__init__(self)
        self.text = getLanguageString(label) or ''
        self.slider = qrangeslider.QRangeSlider(self)

        #if RangeSlider._style is None:
        #    RangeSlider._style = DEFAULT_CSS#
        #self.slider.setBackgroundStyle(RangeSlider._style)
        #self.slider.setSpanStyle(RangeSlider._style)
